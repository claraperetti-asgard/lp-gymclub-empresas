...
const reviews: Review[] = [
  ...
  {
    name: 'Patrícia Fontes',
    initials: 'PF',
    avatarBg: 'bg-gradient-green',
    body: 'Eu tinha três planilhas para controlar academia. Hoje tenho um painel e cinco minutos por mês. O ganho de tempo do RH foi o maior de todos.',
  },
  {
    name: 'Gustavo Lemos',
    initials: 'GL',
    avatarBg: 'bg-gradient-yellow-orange',
    body: 'Testamos duas plataformas antes. As duas cobravam taxa por cadastro. Pagar só por quem treina parece óbvio, mas ninguém mais fazia.',
  },
];
...
function ReviewCard({ name, initials, avatarBg, body }: Review) {
  return (
    <div className="w-[340px] shrink-0 bg-white rounded-3xl p-6 border border-gray-200 shadow-sm">
      ...
      <p className="font-title font-light text-gray-600 leading-relaxed">{body}</p>
    </div>
  );
}
  ...
  return (
    <section className="py-24 relative overflow-x-clip">
      ...
      <div className="relative">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 w-24 md:w-48 bg-gradient-to-r from-[#FAFAFA] to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-24 md:w-48 bg-gradient-to-l from-[#FAFAFA] to-transparent" />
      </div>

      <div className="flex flex-col sm:flex-row items-center justify-center mt-10 gap-4">
            <a href="#cadastro" className="w-full sm:w-auto bg-gradient-green hover:opacity-90 text-gray-900 px-8 py-4 rounded-full text-base font-title font-bold transition-all hover:shadow-xl hover:shadow-gym-green/30 hover:-translate-y-1 flex items-center justify-center gap-2">
